...
function smarty_function_error_tip($params, &$smarty) {
    ...
    $tag = '<div class="tags">&nbsp;&nbsp;&nbsp;<img class="label" src="/common/img/iconfont-tishi-2.png" alt=""/><div class="label-block"><div class="label-text">$message</div><div class="point"></div></div></div>';
    if (empty($errors) || !$tmp) {
        return $result;
    }
    ...
    foreach ($tmp as $key) {
        if ($errors[$key]) {
            ...
            $result .= str_replace('$message', $errors[$key], $tag);
            break;
        }
    }
